...
(() => {
  class Expo {
    constructor(logic){
      this.logic=logic;this.root=[...document.querySelectorAll('#touch-shell')].find(n=>!n.closest('x-dc'));this.abort=new AbortController();
      for(const [event,handler] of [['pointerdown',e=>this.down(e)],['pointermove',e=>this.move(e)],['pointerup',e=>this.up(e)],['pointercancel',()=>this.cancel()],['lostpointercapture',()=>this.cancel()],['click',e=>{if(this.logic.state.ov||Date.now()<this.suppressUntil){e.stopImmediatePropagation();e.preventDefault()}}]])this.root.addEventListener(event,handler,{capture:true,signal:this.abort.signal});
      this.update();
    }
    update(){this.root.classList.toggle('expo-mode',this.logic.state.ov);if(!this.logic.state.ov)this.cancel();else if(this.drag)this.paint()}
    down(e){
    ...
    move(e){
      ...
      if(d.reorder){
        const origin=d.slots[d.order.indexOf(d.key)];const x=origin.x+d.dx,y=origin.y+d.dy;
        let target=1,best=Infinity;d.slots.forEach((slot,i)=>{if(!i)return;const distance=Math.hypot(slot.x-x,slot.y-y);if(distance<best){best=distance;target=i}});
        const open=[...this.logic.state.open],from=open.indexOf(d.key);if(from!==target){open.splice(from,1);open.splice(target,0,d.key);this.logic.set({open,ws:open.indexOf(d.active)})}
      }
      this.paint();
    }
    paint(){const d=this.drag;if(!d?.card||d.key==='home')return;d.card.style.transition='none';d.card.style.zIndex='8';d.card.style.transform=`translate(${(d.reorder?d.dx:0)/d.scale}px,${(d.reorder?d.dy:Math.min(0,d.dy))/d.scale}px) ${d.base}`;d.card.style.opacity=d.reorder?'1':String(Math.max(.2,1-Math.max(0,-d.dy)/350))}
    up(e){
      const d=this.drag;if(!d||e.pointerId!==d.id)return;e.stopImmediatePropagation();e.preventDefault();clearTimeout(this.timer);this.suppressUntil=Date.now()+500;
      if(d.card&&d.key!=='home'&&!d.reorder&&d.dy<-65&&Math.abs(d.dy)>Math.abs(d.dx)*1.2){
        this.drag=null;d.card.classList.remove('expo-lifted');
        const animation=d.card.animate([{transform:d.card.style.transform,opacity:d.card.style.opacity},{transform:`translate(0,-${this.root.clientHeight}px) ${d.base}`,opacity:0}],{duration:190,easing:'cubic-bezier(.3,0,.7,1)'});
