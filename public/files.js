...
(() => {
  ...
  class FilesApp {
    constructor(root, openTerminal, windowKey = 'files') {
      ...
      this.searchField.setAttribute('aria-label', 'Search files');
      this.searchField.autocapitalize = 'none';
      ...
      this.searchField.onkeydown = e => {
        ...
        if (e.key === 'Enter') {
          e.preventDefault();
          if (this.mode === 'fuzzy' && this.results?.length) this.open(this.results[0]);
          else this.search();
        }
      };
      this.scopes = node('div', 'files-scopes');
      this.status = node('div', 'remote-status files-status');
      this.status.setAttribute('role', 'status');
      this.body = node('div', 'files-content');
      this.footer = node('div', 'files-footer');
      this.picker = node('input');
      this.picker.type = 'file';
      this.picker.multiple = true;
      this.picker.hidden = true;
      this.picker.onchange = () => {
        const f = [...this.picker.files];
        this.picker.value = '';
        this.uploadFiles(f, this.uploadPath);
      };
      root.append(
        this.heading,
        this.location,
      ...
      root.addEventListener('pointerdown', e => {
        if (e.target.closest('.files-footer button,.files-scopes button,.files-dialog button'))
          e.preventDefault();
      });
      ...
      root.addEventListener(
        ...
        e => {
          ...
          if (
            ...
            return;
          const t = e.touches[0];
          controlTouch = { button, x: t.clientX, y: t.clientY, moved: false };
          e.preventDefault();
        },
        { passive: false }
      );
      root.addEventListener(
        ...
        e => {
          if (controlTouch) {
            const t = e.touches[0];
            if (Math.hypot(t.clientX - controlTouch.x, t.clientY - controlTouch.y) > 10)
              controlTouch.moved = true;
          }
        },
        { passive: true }
      );
      root.addEventListener(
        ...
        e => {
          ...
          controlTouch = null;
          e.preventDefault();
          if (!touch.moved && touch.button.isConnected && !touch.button.disabled)
            touch.button.click();
        },
        { passive: false }
      );
    ...
    async request(path, options = {}) {
      const r = await fetch('/api/' + path, {
        ...options,
        headers: { 'X-Hyprland-Client': '1', ...options.headers },
        signal: AbortSignal.any([this.abort.signal, AbortSignal.timeout(120000)]),
      });
      if (!r.ok) {
        const data = await r.json().catch(() => ({}));
        throw Error(
          data.error || (r.status === 413 ? 'File is larger than 25 MiB' : 'Request failed')
        );
      }
      return r;
    }
    async json(path, data) {
      return (
        await this.request(
          path,
          data === undefined
    ...
    short(path) {
      return this.homePath && path.startsWith(this.homePath)
        ? '~' + path.slice(this.homePath.length)
        : path;
    }
    remember(e) {
    ...
    async browse(path, keepMode = false) {
      ...
      this.drawChrome();
      try {
        const data = await this.json('files?' + new URLSearchParams({ path, hidden: this.hidden }));
        if (seq !== this.sequence || this.disposed) return;
        this.path = data.path;
        this.homePath = data.root;
        ...
        if (seq === this.sequence && !this.disposed) {
          ...
          this.body.replaceChildren(this.button('Try again', () => this.browse(path)));
        }
      }
    }
    crumbs() {
      this.location.replaceChildren();
      this.location.title = this.path;
      this.location.dataset.path = this.path;
      const relative = this.homePath ? this.path.slice(this.homePath.length) : '';
      let part = this.homePath || '';
      this.location.append(this.button('~', () => this.browse(''), 'Home folder'));
      for (const segment of relative.split('/').filter(Boolean)) {
        part += '/' + segment;
        const target = part;
        this.location.append(
          node('span', 'files-chevron', '›'),
    ...
    drawChrome() {
      this.root.classList.toggle('files-previewing', !!this.previewing);
      this.heading.replaceChildren();
      this.searchBox.hidden = this.previewing || this.selecting;
      this.scopes.hidden = this.previewing;
      this.location.hidden = this.mode === 'fuzzy' && !this.previewing;
      ...
      if (this.selecting) {
        ...
        this.scopes.replaceChildren(
          this.button('select all', () => this.selectAll()),
          this.button('invert', () => this.selectAll(true)),
          this.button('pattern', () => this.patternDialog(), 'Select by pattern')
        );
      } else {
        this.searchChrome();
      }
      ...
      if (this.selecting) {
        ...
        this.footer.append(
          node(
            ...
            `${chosen.length} items · ${size(chosen.filter(e => !e.directory).reduce((n, e) => n + e.size, 0))} · ZIP on download`
          )
        );
        ...
        for (const [label, key] of [
          ...
          b.setAttribute('aria-pressed', String(key === 'sensitive' ? !this[key] : this[key]));
    ...
    searchChrome() {
      if (this.previewing || this.selecting) return;
      this.scopes.replaceChildren();
      this.searchField.placeholder =
        this.mode === 'fuzzy' ? `fuzzy find on ${HyprlandApps.host.name}…` : 'search this folder…';
      this.prefix.textContent =
        this.mode === 'fuzzy' ? '›' : this.searchMode === 'names' ? '/' : 'rg';
      if (this.mode === 'fuzzy') {
        this.scopes.append(
          node('span', 'files-chip', 'fuzzy'),
        ...
        for (const mode of ['names', 'contents', 'everywhere']) {
          ...
          b.setAttribute('aria-pressed', String(this.searchMode === mode));
          this.scopes.append(b);
        }
      }
    ...
    drawList() {
      ...
      if (this.mode === 'fuzzy' && !this.selecting) {
        this.drawRecents();
        return;
      }
    ...
    entry(e, subpath = false) {
      const row = this.button('', () => {
        ...
        this.selecting ? this.toggle(e) : this.open(e);
      });
      row.className =
        'files-entry' +
      ...
      if (subpath)
        name.append(node('small', '', this.short(e.path.slice(0, e.path.lastIndexOf('/')))));
      row.append(
        icon,
        name,
        node('span', 'files-meta', e.directory ? (e.count ?? '') : size(e.size || 0))
      );
    ...
    highlight(el, text) {
      ...
      for (const m of text.matchAll(re)) {
        if (!m[0]) continue;
        el.append(document.createTextNode(text.slice(from, m.index)), node('mark', '', m[0]));
    ...
    toggle(e) {
      ...
      else if (this.selected.size < 100) this.selected.add(e.path);
      else this.status.textContent = 'Select up to 100 items';
      this.drawChrome();
      this.drawList();
    }
    selectAll(invert = false) {
      ...
      for (const e of this.entries) {
        if ((!invert || !before.has(e.path)) && this.selected.size < 100) this.selected.add(e.path);
      }
      this.drawChrome();
      this.drawList();
    }
    ...
    setMode(mode) {
      ...
      ++this.sequence;
      this.mode = mode;
      store('omarchy-files-mode', mode);
      this.query = '';
      this.searchField.value = '';
      this.drawChrome();
      this.drawList();
      if (mode === 'fuzzy') this.searchField.focus();
    }
    async search() {
      if (!this.query) {
        this.drawChrome();
      ...
      try {
        const data = await this.json(
          'files/search?' +
            new URLSearchParams({
              path: this.path,
              query: this.query,
              mode: this.mode === 'fuzzy' ? 'fuzzy' : this.searchMode,
              hidden: this.hidden || this.mode === 'fuzzy',
              regex: this.mode !== 'fuzzy' && this.regex,
              sensitive: this.mode !== 'fuzzy' && this.sensitive,
              glob: this.mode === 'fuzzy' ? '' : this.glob,
            })
        );
        if (seq !== this.sequence || this.disposed) return;
        ...
        for (const e of data.entries) {
          ...
          for (const hit of e.lines || []) {
            ...
            row.className = 'files-search-hit';
            row.append(node('span', 'files-line-number', hit.number));
            const text = node('span');
            this.highlight(text, hit.text);
            row.append(text);
    ...
    async preview(entry, line = 0) {
      ...
      this.path = entry.path.slice(0, entry.path.lastIndexOf('/'));
      ...
      try {
        if (entry.size > 25 * 1024 * 1024)
          throw Error('Files over 25 MiB cannot be transferred here');
        const data = await (
          await this.request('files/content?' + new URLSearchParams({ path: entry.path }))
        ).arrayBuffer();
        if (seq !== this.sequence || this.disposed) return;
        const mime = {
          png: 'image/png',
        ...
        this.file = new File([data], entry.name, { type: mime || 'application/octet-stream' });
        ...
        if (mime) {
          const image = node('img', 'files-image');
          image.alt = entry.name;
          image.src = this.url;
          this.body.append(image);
        } else if (data.byteLength <= 1024 * 1024 && !new Uint8Array(data).includes(0)) {
          try {
            const text = await this.json('files/text?' + new URLSearchParams({ path: entry.path }));
            if (seq !== this.sequence || this.disposed) return;
            this.text = text.text;
            ...
            this.body.append(node('p', 'remote-empty', 'Text preview unavailable: ' + e.message));
          }
        } else
          this.body.append(
            node('p', 'remote-empty', 'Preview unavailable. You can still save this file.')
          );
        this.previewFooter();
      } catch (e) {
        if (seq === this.sequence && !this.disposed) this.status.textContent = e.message;
      }
    }
    drawCode(line = 0) {
      this.code.replaceChildren();
      this.code.classList.toggle('wrap', !!this.wrap);
      const lines = this.text.split('\n');
      for (let i = 0; i < Math.min(lines.length, this.limit); i++) {
        const row = node('div', 'files-code-row' + (i + 1 === line ? ' highlighted' : ''));
        row.dataset.line = i + 1;
        row.append(node('span', 'files-line-number', i + 1));
        const text = node('span', 'files-code-source');
        this.syntax(text, lines[i]);
        row.append(text);
    ...
    syntax(el, text) {
      const re =
        /(#[^\n]*|\/\/[^\n]*|"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|\b(?:const|let|var|function|return|if|else|import|from|pub|fn|use|struct|true|false|null)\b|\b\d+(?:\.\d+)?\b)/g;
      let at = 0;
      for (const m of text.matchAll(re)) {
        el.append(document.createTextNode(text.slice(at, m.index)));
        const token = m[0],
          cls =
            token.startsWith('#') || token.startsWith('//')
              ...
              : /^["']/.test(token)
                ...
                : /^\d/.test(token)
                  ? 'number'
                  : 'keyword';
        el.append(node('span', 'syntax-' + cls, token));
      ...
      try {
        if (window.__HYPRLAND_NATIVE__) {
          if (!navigator.canShare?.({ files: [file] }))
            throw Error('Saving is unavailable in this view.');
        ...
        if (e.name !== 'AbortError') this.status.textContent = e.message;
      }
    }
    edit() {
      this.editing = true;
      this.editor = node('textarea', 'files-editor');
      ...
      this.code.replaceWith(this.editor);
      this.footer.replaceChildren(
        this.button('save', () => this.saveEdit(), 'Save changes'),
        this.button(
          'cancel',
          () => {
    ...
    cancelEdit() {
      this.editor.replaceWith(this.code);
      this.editing = false;
      this.blur();
      this.previewFooter();
    }
    async saveEdit() {
      ...
      try {
        const data = await this.json('files/text', {
          path: this.entryData.path,
          text: draft,
          version: this.version,
        });
        this.version = data.version;
        this.text = draft;
        this.file = new File([this.text], this.entryData.name, { type: 'text/plain' });
        this.entryData.size = this.file.size;
        this.remember(this.entryData);
        this.details.textContent = [
          size(this.file.size),
          ...this.details.textContent.split(' · ').slice(1),
        ].join(' · ');
        this.cancelEdit();
        this.drawCode();
        this.status.textContent = `Saved to ${HyprlandApps.host.name}.`;
      } catch (e) {
        this.status.textContent = e.message;
      } finally {
        this.saving = false;
    ...
    async uploadFiles(files, path) {
      ...
      for (const file of files) {
        ...
        try {
          if (file.size > 25 * 1024 * 1024) throw Error('larger than 25 MiB');
          await this.request('files/upload?' + new URLSearchParams({ path, name: file.name }), {
            method: 'POST',
            body: file,
          });
          messages.push('Uploaded ' + file.name);
        } catch (e) {
          messages.push(file.name + ': ' + e.message);
        }
      }
      this.uploading = false;
      if (this.disposed) return;
      if (this.path === path && !this.previewing) await this.browse(path);
        ...
        [
          ...
          () =>
            this.form(
              ...
              async ({ name }) => {
                await this.request(
                  'files/upload?' + new URLSearchParams({ path: this.path, name }),
    ...
    async copyPath(path) {
      try {
        ...
        this.status.textContent = 'Path copied.';
      } catch {
        this.form('Copy path', [['path', 'Path', path]], async () => {}, 'Done');
      }
    }
    terminalHere() {
      const run = async () => {
        await this.openTerminal?.(this.path);
      };
      this.confirm(
        'Terminal here',
        'Open a new shell in this folder? This replaces the app-owned Terminal shell if one is open.',
        run,
        'Open shell'
      );
    }
    async getSelected() {
      try {
        this.status.textContent = 'Preparing ZIP…';
        const blob = await (
          await this.request('files/archive', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ paths: [...this.selected] }),
          })
        ).blob();
        const file = new File([blob], 'files.zip', { type: 'application/zip' });
        this.confirm(
          'ZIP ready',
          size(blob.size),
          () => this.saveFile(file),
        ...
        this.status.textContent = 'ZIP ready.';
      } catch (e) {
        this.status.textContent = e.message;
      }
    }
    operation(action, paths = [...this.selected]) {
      const perform = async extras => {
        const result = await this.json('files/operate', { action, paths, ...extras });
        await this.browse(this.path);
        this.status.textContent =
          `${result.completed.length} completed` +
          (result.errors.length ? ' · ' + result.errors.map(e => e.error).join(' · ') : '');
      };
      if (action === 'trash')
        this.confirm(
          'Move to Trash?',
        ...
        this.form(
          'Rename item',
          [['name', 'New name', paths[0].split('/').pop()]],
          perform,
          'Rename'
        );
      else
        this.form(
          ...
          [['destination', 'Destination folder', this.path]],
          perform,
    ...
    form(title, fields, submit, label) {
      ...
      for (const [key, aria, value] of fields) {
        ...
        field.value = value;
        field.setAttribute('aria-label', aria);
        field.placeholder = aria;
      ...
      form.onsubmit = async e => {
        ...
        save.disabled = true;
        try {
          await submit(Object.fromEntries(Object.entries(inputs).map(([k, v]) => [k, v.value])));
          this.blur();
          this.closeDialog();
        } catch (e) {
          this.status.textContent = e.message;
