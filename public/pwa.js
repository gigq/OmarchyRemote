...
function fitCanvas() {
  ...
  window.dispatchEvent(
    new CustomEvent('hyprland-layout', {
      detail: {
        desk,
        width: viewport.clientWidth,
        height: viewport.clientHeight,
        inset: inset > 80 ? inset : 0,
      },
    })
  );
}
...
document.addEventListener(
  'pointerdown',
  event => {
    touch = event.target.closest('#touch-shell')
...
document.addEventListener(
  'pointermove',
  event => {
    if (touch && Math.hypot(event.clientX - touch.x, event.clientY - touch.y) > 15)
      touch.moved = true;
  },
  true
);
...
window.addEventListener('hyprland-keyboard', () => {
  fitCanvas();
  fitNativeKeyboard();
});
...
function updateDeviceBattery() {
  ...
  document.querySelectorAll('[data-device-battery]').forEach(el => {
    ...
    const percent = Math.round(battery.percent),
      charging = ['charging', 'full'].includes(battery.state);
    el.dataset.charging = String(charging);
    el.dataset.low = String(percent <= 20);
