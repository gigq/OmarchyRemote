...
(() => {
  ...
  class TerminalApp {
    constructor(root,bridge){
      ...
      const bar=node('div','remote-bar');bar.append(this.status,this.restart);root.append(bar,this.host);
      this.term=terminal(this.host);this.fit=new FitAddon.FitAddon();this.term.loadAddon(this.fit);
    ...
    input(data){if(this.ready&&this.ws?.readyState===1){this.ws.send(JSON.stringify({type:'input',data}));this.term.scrollToBottom()}else this.status.textContent=this.exited?'Shell exited · start a new shell':'Disconnected · input was not sent';}
    dispose(){this.disposed=true;clearTimeout(this.retry);this.ws?.close();this.resizeObserver.disconnect();this.term.dispose()}
  }
  class HerdrApp {
    constructor(root,bridge){
      ...
      const inputBar=node('div','herdr-input-bar');inputBar.append(button('⌨ Keyboard',()=>bridge.keyboard()),this.inputStatus);
      this.detail.append(this.detailBar,this.output,inputBar);this.term=terminal(this.canvas,true);
      this.output.onclick=()=>bridge.keyboard();
    ...
    select(id){
      this.selected=id;storage.set('omarchy-herdr-pane',id);this.lastRead=null;this.term.reset();this.inputStatus.textContent='Tap to type into this pane';
    ...
    renderOutput(read,force=false){
      ...
      const rows=Math.max(4,Math.floor(this.output.clientHeight/14));
      const bottom=this.term.buffer.active.viewportY>=this.term.buffer.active.baseY;const scroll=this.term.buffer.active.viewportY;
      this.canvas.style.width=`${Math.max(this.output.clientWidth,cols*7.23)}px`;
      this.term.resize(cols,rows);this.term.reset();this.term.write(text.replace(/\r?\n/g,'\r\n'),()=>{if(bottom||!previous)this.term.scrollToBottom();else this.term.scrollToLine(scroll)});
    }
    input(input){
