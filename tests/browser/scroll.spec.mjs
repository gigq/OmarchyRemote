...
async function herdr(method, params) {
  return new Promise((resolve, reject) => {
    const s = net.createConnection(`${process.env.HOME}/.config/herdr/herdr.sock`);
    let text = '';
    s.setTimeout(8000);
    s.on('connect', () =>
      s.write(JSON.stringify({ id: 'mobile-scroll-test', method, params }) + '\n')
    );
    s.on('data', chunk => {
      text += chunk;
      if (text.includes('\n')) {
        s.end();
        const r = JSON.parse(text.split('\n')[0]);
...
async function state(page) {
  return page.evaluate(() => {
    ...
    return {
      top: b.viewportY,
      bottom: b.baseY,
      first: b.getLine(b.viewportY)?.translateToString(true),
      last: b.getLine(b.length - 1)?.translateToString(true),
    };
  });
}
async function drag(page, from, to) {
  ...
  for (let i = 1; i <= 6; i++) {
    await cdp.send('Input.dispatchTouchEvent', {
      type: 'touchMove',
      touchPoints: [{ x: 200, y: from + ((to - from) * i) / 6 }],
    });
...
test('Herdr touch history stays anchored during updates and typed text is visible', async ({
  ...
  try {
    const created = await herdr('workspace.create', {
      label: 'Mobile scrolling test',
      cwd: '/tmp',
      focus: false,
    });
    workspace = created.workspace.workspace_id;
    const snap = (await herdr('session.snapshot', {})).snapshot;
    const pane = snap.panes.find(p => p.workspace_id === workspace).pane_id;
    ...
    await page
      .locator('.herdr-group')
      .filter({ hasText: 'Mobile scrolling test' })
      .locator('.herdr-pane')
      .click();
    await expect.poll(async () => (await state(page)).bottom).toBeGreaterThan(90);
    await drag(page, 300, 650);
    await expect
      .poll(async () => {
        const s = await state(page);
        return s.bottom - s.top;
    ...
    await held.send('Input.dispatchTouchEvent', {
      type: 'touchStart',
      touchPoints: [{ x: 200, y: 400 }],
    });
    const heldBottom = (await state(page)).bottom;
    await herdr('pane.send_input', { pane_id: pane, text: 'seq 161 240', keys: ['Enter'] });
    await page.waitForTimeout(1000);
    expect((await state(page)).bottom).toBe(heldBottom);
    await held.send('Input.dispatchTouchEvent', { type: 'touchCancel', touchPoints: [] });
    await held.detach();
    ...
    expect(
      await page
        .locator('.herdr-output')
        .evaluate(
          el =>
            el.querySelector('.xterm-screen').getBoundingClientRect().bottom <=
