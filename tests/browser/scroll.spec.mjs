...
async function captureTerminals(page){await page.addInitScript(()=>{let ctor;Object.defineProperty(window,'Terminal',{configurable:true,get:()=>ctor,set:Base=>{ctor=class extends Base{constructor(...args){super(...args);(window.qaTerms??=[]).push(this)}}}})})}
...
test('Terminal touch drags read history without opening the keyboard',async({page})=>{
 ...
 }finally{await page.evaluate(()=>qaTerms[0].input('\x03\x15exit\r'))}
});
...
test('Herdr touch history stays anchored during updates and typed text is visible',async({page})=>{
 ...
 try{
  ...
  await expect(page.locator('.herdr-canvas .xterm-rows')).toContainText('echo hello');
