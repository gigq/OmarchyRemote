...
test('btop has a dedicated monitor session, touch controls and survives reload', async ({
  ...
  const id = await p.evaluate(() => localStorage.getItem('omarchy-btop-id'));
  ...
  await expect(app.locator('.native-terminal-row span').filter({ hasText: '─' }).first()).toHaveCSS(
    'color',
    await p.evaluate(() => {
  ...
  // Reload restores the open workspace and reconnects the same monitor session.
  await p.reload();
  await expect(app).toContainText('· connected');
