...
test('native output preserves ANSI colors, wide cells and literal markup', async ({ page }) => {
  ...
  await expect(rows).toContainText('界🙂');
  await expect(rows).toContainText('<script>unsafe</script>');
  ...
  ).toBe('rgb(235, 111, 146)');
  const inverse = rows.locator('span').filter({ hasText: 'inverse' });
...
test('scroll position moves by pixels and DOM stays bounded for long history', async ({ page }) => {
  ...
  await page.evaluate(
    () =>
      new Promise(resolve =>
        qaTerms[0].write(Array.from({ length: 2900 }, (_, i) => `row ${i}\r\n`).join(''), resolve)
      )
  );
  ...
  const expectedFirst = await page.evaluate(
    () => Math.floor(20003 / qaTerms[0].nativeView.height) - 80
  );
  await expect(scroller.locator('.native-terminal-row').first()).toHaveAttribute(
    'data-line',
  ...
  expect(await scroller.locator('.native-terminal-row').first().getAttribute('data-line')).toBe(
    first
  );
});

test('keyboard viewport resize keeps latest pinned and preserves a history position', async ({
  page,
}) => {
  await open(page);
  await page.evaluate(
    () =>
      new Promise(resolve =>
        qaTerms[0].write(Array.from({ length: 220 }, (_, i) => `resize ${i}\r\n`).join(''), resolve)
      )
  );
  const scroller = page.locator('.native-terminal-scroll');
  await expect
...
test('alternate screen and cursor visibility follow terminal escape sequences', async ({
  page,
}) => {
  await open(page);
  const content = page.locator('.native-terminal-content');
  await page.evaluate(
    () =>
      new Promise(resolve =>
        qaTerms[0].write('\x1b[?1049h\x1b[2J\x1b[HALTERNATE\x1b[?25l', resolve)
      )
  );
  await expect(content).toContainText('ALTERNATE');
  await expect(content).not.toContainText('renderer fixture');
  await expect.poll(() => content.locator('span[style*="box-shadow"]').count()).toBe(0);
  await page.evaluate(
    () => new Promise(resolve => qaTerms[0].write('\x1b[?1049l\x1b[?25h', resolve))
  );
  await expect(content).toContainText('renderer fixture');
  await expect(content).not.toContainText('ALTERNATE');
