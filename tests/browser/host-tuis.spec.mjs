...
for(const app of ['lazydocker','dua','lnav'])test(`${app} opens real host output, controls and restores its session`,async({page:p})=>{
 ...
 try{
  ...
  const root=p.locator(`#remote-${app}-app`);await expect(root).toContainText('· connected');
  ...
  await p.reload();await p.getByText(app,{exact:true}).first().click();await expect(root).toContainText('· connected');
