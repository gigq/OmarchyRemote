...
test.beforeEach(async ({ page }) => {
  ...
  await page.evaluate(() => {
    window.sent = [];
    window.keys = [];
    window.input = new NativeInput(document.getElementById('input'), {
      message: true,
      send: (text, enter) => {
        if (window.offline) return false;
        sent.push({ text, enter });
        return true;
      },
      key: (key, mods) => keys.push({ key, mods }),
      focus: () => {},
      hide: () => {},
    });
    input.focus();
...
test('message edits remain local, retain failed drafts and stay with their pane', async ({
  page,
}) => {
  await expect(page.locator('.native-input-tools')).toBeHidden();
  const field = page.locator('.native-input');
  await page.evaluate(() => input.select('pane-one'));
  await field.fill('helo');
  await field.fill('hello');
  expect(await page.evaluate(() => sent)).toEqual([]);
  await page.evaluate(() => input.select('pane-two'));
  await expect(field).toHaveValue('');
  await field.fill('second pane');
  await page.evaluate(() => input.select('pane-one'));
  ...
  await field.press('Enter');
  expect(await page.evaluate(() => sent)).toEqual([{ text: 'hello', enter: true }]);
  await expect(field).toHaveValue('');
...
test('direct keys, delete, Ctrl and IME commit are sent once', async ({ page }) => {
  ...
  await field.evaluate(el => {
    ...
    el.value = '\u200b日本';
    el.dispatchEvent(new InputEvent('input', { data: '日本', isComposing: true }));
    el.dispatchEvent(new CompositionEvent('compositionend', { data: '日本' }));
    el.dispatchEvent(new InputEvent('input', { data: '日本', inputType: 'insertText' }));
  });
  expect(await page.evaluate(() => sent.map(s => s.text).join(''))).toBe('abc日本');
  expect(await page.evaluate(() => keys)).toEqual([
    { key: '⌫', mods: {} },
    { key: '⏎', mods: {} },
    { key: 'c', mods: { ctrl: true } },
  ]);
});
test('image paths append to the original draft without sending or replacing text', async ({
  page,
}) => {
  const field = page.locator('.native-input');
  ...
  await field.press('Enter');
  expect(await page.evaluate(() => sent)).toEqual([]);
  await page.evaluate(() => (input.submit.disabled = false));
  await field.press('Enter');
  expect(await page.evaluate(() => sent)).toEqual([
    { text: 'Look at this\nImage: /tmp/image-one.png\nImage: /tmp/image-two.png\n', enter: true },
  ]);
});
for (const trigger of ['button', 'keyboard', 'beforeinput'])
  test(`Herd dismisses after ${trigger} send but retains failed drafts`, async ({ page }) => {
    await page.evaluate(() => {
...
test('Herd drafts survive page reload per pane, including composition and background image attachments', async ({
  page,
}) => {
  const mount = async () => {
    await page.setContent('<div id="input"></div>');
    ...
    await page.evaluate(() => {
      ...
      window.input = new NativeInput(document.getElementById('input'), {
        message: true,
        draftStore: 'omarchy-herdr-drafts-v1',
        send: text => {
          if (window.offline) return false;
          sent.push(text);
          return true;
  ...
  await page.reload();
  await mount();
  await expect(field).toHaveValue('First unfinished message\nImage: /tmp/attached.png\n');
  await page.evaluate(() => {
    input.select('agent-b');
    window.offline = true;
  });
  await field.press('Enter');
  await expect(field).toHaveValue('Second message');
  await page.reload();
  await mount();
  await page.evaluate(() => input.select('agent-b'));
  await expect(field).toHaveValue('Second message');
  ...
  await page.evaluate(() => input.select('agent-b'));
  await expect(field).toHaveValue('');
  await page.evaluate(() => input.select('agent-a'));
  await expect(field).toHaveValue('First unfinished message\nImage: /tmp/attached.png\n');
});

test('unavailable draft storage warns without losing or sending the current text', async ({
  page,
}) => {
  await page.addScriptTag({ url: '/util.js' });
  await page.evaluate(() => {
    input.draftStore = 'omarchy-herdr-drafts-v1';
    input.select('agent-a');
