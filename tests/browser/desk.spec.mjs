...
const boot=async p=>{await p.route('**/api/**',r=>r.abort());await p.goto('/native/');await expect(p.locator('html')).toHaveClass(/desk-mode/);await expect(label(p)).toHaveText('home')};
...
test.describe('landscape iPad',()=>{
 ...
 test('shortcuts open windows into dwindle tiles, focus, fullscreen, move, and close them',async({page:p})=>{
  ...
  await expect(label(p)).toHaveText('terminal');await expect(pills(p)).toHaveCount(2);
  await p.keyboard.press('Meta+Shift+Enter');
  await expect(label(p)).toHaveText('browser');await expect(pills(p)).toHaveCount(2);
  ...
  await p.keyboard.press('Meta+ArrowLeft');await expect(label(p)).toContainText('terminal');
  await p.keyboard.press('Meta+ArrowRight');await expect(label(p)).toContainText('browser');
  await p.keyboard.press('Meta+ArrowDown');await expect(label(p)).toContainText('herdr');
  await p.keyboard.press('Meta+f');await expect(label(p)).toHaveText('herdr');
  ...
  await expect(pills(p)).toHaveCount(3);await expect(label(p)).toHaveText('herdr');
  await p.keyboard.press('Meta+Digit2');await expect(label(p)).toHaveText('terminal');
  await p.keyboard.press('Meta+Digit1');await expect(label(p)).toHaveText('home');
  await p.keyboard.press('Meta+BracketRight');await expect(label(p)).toHaveText('terminal');
  ...
  await p.keyboard.press('Meta+ArrowRight');await expect(label(p)).toContainText('browser');
  await p.keyboard.press('Meta+w');await expect(label(p)).toHaveText('terminal');
  await p.keyboard.press('Meta+Shift+A');await expect(label(p)).toHaveText('herdr');await expect(pills(p)).toHaveCount(3);
  await p.keyboard.press('Meta+Backspace');await expect(label(p)).toHaveText('terminal');
 ...
 test('Home fills the desk with a framed clock panel beside the widget column',async({page:p})=>{
  ...
  expect(home.width).toBeGreaterThan(1100);expect(home.y).toBe(54);
