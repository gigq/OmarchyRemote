...
test('active terminal accepts typing immediately and exited tabs/windows disappear',async({page:p})=>{
 ...
 await p.keyboard.press('Meta+Comma');await p.keyboard.press('Meta+ArrowLeft');await expect(p.locator('.terminal-tab .native-input')).toBeFocused();
 ...
 await p.locator('.terminal-tab .native-input').evaluate(el=>el.blur());await expect(p.locator('.terminal-tab .native-input')).toBeFocused();
 await p.keyboard.press('Meta+e');await p.keyboard.press('Escape');await expect(p.locator('.terminal-tab .native-input')).toBeFocused();
 await p.keyboard.type('again');await expect.poll(()=>inputs.join('')).toBe('helloagain');
