...
test('all Omarchy themes apply to shell and ANSI output and persist', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Catppuccin Latte', exact: true })).toHaveAttribute(
    'aria-pressed',
    'true'
  );
  ...
  const result = await page.evaluate(async () => {
    ...
    host.style.cssText = 'position:fixed;inset:100px 20px;height:200px';
    document.body.append(host);
    const term = new Terminal({ cols: 40, rows: 5, theme: HyprlandThemes.terminalTheme() });
    term.open(host);
    const view = new NativeTerminalView(host, term);
    await new Promise(r => term.write('\x1b[31mRed\x1b[0m plain', r));
    HyprlandThemes.apply('catppuccin-latte');
    await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));
    const span = host.querySelector('.native-terminal-row span');
    const value = {
      color: span.style.color,
      background: term.options.theme.background,
      text: host.querySelector('.native-terminal-content').textContent,
    };
    view.dispose();
    term.dispose();
    host.remove();
  ...
  expect(result.color).toBe('rgb(210, 15, 57)');
  expect(result.background).toBe('#e3e4e8');
