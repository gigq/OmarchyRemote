...
test('fit wraps words and wide glyphs without changing source columns or losing ANSI text', async ({
  page,
}) => {
  ...
  const prose =
    'This is a long paragraph with colored words that should wrap cleanly at spaces without changing the original desktop terminal layout.';
  await page.evaluate(async prose => {
    const t = qaTerms[0];
    t.resize(180, 30);
    t.reset();
    await new Promise(r =>
      t.write('\x1b[31m' + prose + '\x1b[0m\r\n' + '界🙂'.repeat(50) + '\r\n' + 'x'.repeat(170), r)
    );
    t.nativeView.setFit(true);
  }, prose);
...
test('fitted scrolling retains its source line when toggling width modes', async ({ page }) => {
    ...
    await new Promise(r =>
      t.write(
        Array.from(
          { length: 100 },
          (_, i) =>
            `Line ${i} with a long readable paragraph that will fill several mobile rows while keeping its original source coordinates.\r\n`
        ).join(''),
        r
      )
    );
  ...
  const scroller = page.locator('.native-terminal-scroll');
  await expect.poll(() => scroller.evaluate(el => el.scrollHeight)).toBeGreaterThan(4000);
  ...
  await page.evaluate(() => qaTerms[0].nativeView.setFit(true));
  await page.waitForTimeout(100);
  expect(await page.evaluate(() => qaTerms[0].nativeView.anchor().source)).toBe(source);
});
