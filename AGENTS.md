...
- Publish every native build you install on a device, in the same task, so the Builds dashboard's newest build always matches the devices; otherwise the in-app **Install** offers an older build and downgrades them. Raise the build number first (`CURRENT_PROJECT_VERSION` for iOS, `-PversionCode` for Android). For iOS, build for a device or Any iOS Device, run `scripts/package-ios-app.sh <App.app> <out.ipa>` on the build Mac, and publish that IPA; for Android, publish the APK you installed. Use `scripts/publish-native-build.py` with the source commit and release notes and `OMARCHY_DOWNLOAD_BASE_URL` or an explicit private HTTPS base URL; retain artifacts outside Git and report hosting separately from installation.
...
- `public/remote.js` and `public/remote.css` own the real app views. `public/native-terminal.js` renders xterm buffers into native overflow views; `public/native-input.js` owns system-keyboard input and pane drafts. `public/dashboard.js` and `public/dashboard.css` own Home summaries, pinned apps, notifications, and the native-input launcher. The exported component owns shell gestures. Text entry uses native inputs; there is no custom shell keyboard.
