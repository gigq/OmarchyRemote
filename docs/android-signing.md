...
For live host validation, supply `ANDROID_RELEASE_HOST_URL` with your private HTTPS host address. The fixture verifies live Home on both signed versions using the native UI. It checks for disabled runtime WebView debugging on production Android images; userdebug images force that debugging in the provider and are reported explicitly instead.
