...
- **App windows.** `--app=<key>` opens one catalog app in its own window, loaded as `/native/?app=<key>`. Launching the client again hands the request to the running process, which opens that app's window or focuses it if it is already open; without `--app` it opens or focuses the full shell. Each window remembers its size and position. Settings › app windows lists every app with **open** and, on Linux, **launcher**, which writes or removes `~/.local/share/applications/omarchy-remote-<key>.desktop` so the app appears in the desktop's application launcher. Switching hosts reloads every window on the new host.
...
Loading the shell with `?app=<key>` (for example `/native/?app=herdr`) shows only that app, filling the window. There is no top bar, Home, workspace frame, wallpaper, home indicator, Expo, launcher or window management, and the app's own keys and controls are unchanged. The layout is always the 1:1 desk layout, even in a narrow window. Workspace, window and launcher shortcuts and gestures do nothing; opening another app, for example Files' Terminal here, opens it in its own window (a desktop window in the desktop client, a new tab in a browser) and leaves this window's app alone. Closing the app, for example when its shell exits, closes the window. A single-app window never restores or saves the shell's workspace layout; it reads only your shortcut overrides. App state such as Herdr's selected pane, drafts and host sessions is shared with the full shell, so a Terminal window attaches to the same host shell. An unknown key says so instead of showing the shell.

Settings runs as its own window too (`?app=settings`). The theme and wallpaper are device settings shared by every window on the device: choosing one in any Settings window repaints every other open window at once, whether a single-app window, the full shell, or another browser tab.
