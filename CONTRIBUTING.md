...
A provider can register `widgets: [{ key, name, source, render(root, snapshot, context) }]` alongside `create`. Load that provider module before `widget-deck.js`. `HyprlandApps.widgets()` adds the owning app key automatically; the Home widget manager uses its name and tapping its content opens that app. `source` selects the field in `/api/widgets` (defaults to the widget key). For a separate data source, set `endpoint` to a path under `/api/` instead of `source`; requests are deduplicated per endpoint and failures are isolated. The render context supplies `error` and `open(action)`; the latter opens the owning app before calling `action(instance)`, so widget rows can select a thread or other item. The shared Home widget controller creates the widget root as `#widget-<key>`; render handles null before the first sample. Keep app-specific rendering in the provider module, following `public/codexbar.js`. Controls inside a widget are excluded from launch, as are drags and long presses.
