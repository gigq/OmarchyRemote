...
test('live server serves native assets without caching and confines file reads', async () => {
  ...
  for (const url of ['/native/..%2f..%2fREADME.md', '/.git/config', '/native/%00', '/missing', '/native/sw.js']) {
...
test('a saved source file emits a reload event and the native copy updates', { timeout: 10000 }, async () => {
  ...
  try {
    ...
    async function nextVersion() {
      while (true) {
        ...
        if (match) { buffered = buffered.slice(match.index + match[0].length); return match[1]; }
    ...
    await writeFile(file, 'live reload probe', { flag: 'wx' }); created = true;
    const after = await nextVersion();
    assert.notEqual(after, before);
    assert.equal(await (await fetch(base + '/native/' + file.slice('public/'.length))).text(), 'live reload probe');
    await unlink(file); created = false;
    await nextVersion();
    assert.equal((await fetch(base + '/native/' + file.slice('public/'.length))).status, 404);
  } finally { controller.abort(); if (created) await unlink(file); }
});
...
test('client reloads on new versions, reconnects, and foreground updates', async () => {
  ...
  function createContext() {
    count = 0;
    return { window: { __HYPRLAND_VERSION__: '1' }, location: { reload() { count++; } },
      EventSource: class { constructor(url) { assert.equal(url, '/__dev/events'); } addEventListener(name, callback) { events = callback; } },
      document: { visibilityState: 'visible', addEventListener(name, callback) { foreground = callback; } },
      fetch: async () => ({ ok: true, json: async () => ({ version: '2' }) }) };
  }
  vm.runInNewContext(source, createContext());
  events({ data: '1' }); assert.equal(count, 0);
  events({ data: '2' }); events({ data: '3' }); assert.equal(count, 1);
  vm.runInNewContext(source, createContext());
  await foreground(); assert.equal(count, 1);
  // A restarted server delivers a new version on the first/reconnected event.
  vm.runInNewContext(source, createContext());
  events({ data: 'restarted' }); assert.equal(count, 1);
});
