...
const allowedHosts = new Set(
  (process.env.OMARCHY_ORIGINS || 'http://127.0.0.1:4187,http://localhost:4187')
    .split(',')
...
function proxyApi(req, res) {
  if (!proxyToken || !allowedHosts.has(req.headers.host)) {
    res.writeHead(403);
...
function prepareNative() {
  execFileSync(process.env.PYTHON || 'python3', [path.join(root, 'scripts/prepare-native.py')], {
    cwd: root,
    stdio: 'pipe',
  });
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/__dev/live-reload.js') {
      ...
      res.end(req.method === 'HEAD' ? undefined : liveScript);
      return;
    }
    if (url.pathname === '/native') {
      res.writeHead(307, { ...headers, Location: '/native/' });
      res.end();
      return;
    }
    ...
    let relative = decodeURIComponent(
      native ? url.pathname.slice('/native/'.length) : url.pathname.slice(1)
    );
    ...
    if (!requested.startsWith(base + path.sep) || !existsSync(requested)) {
      res.writeHead(404, headers);
      res.end('Not found');
      return;
    }
    const actual = await realpath(requested);
    if (!actual.startsWith(base + path.sep) || !(await stat(actual)).isFile()) {
      res.writeHead(404, headers);
      res.end('Not found');
      return;
    }
    ...
    if (path.extname(actual) === '.html') {
      ...
      body = Buffer.from(
        body
          ...
          .replace(
            '</head>',
            `<script>window.__HYPRLAND_DEV__ = true; window.__HYPRLAND_VERSION__ = ${JSON.stringify(documentVersion)};</script><script src="/__dev/live-reload.js" defer></script></head>`
          )
      );
    }
    res.writeHead(200, {
...
server.on('upgrade', (req, socket, head) => {
  if (!req.url.startsWith('/api/') || !proxyToken || !allowedHosts.has(req.headers.host)) {
