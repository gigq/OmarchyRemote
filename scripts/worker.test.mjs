...
const request = (pathname, method = 'GET') => worker.fetch(new Request(`https://hyprland.test${pathname}`, { method }));
test('install metadata and every precached asset are served with valid types', async () => {
  ...
  for (const match of html.matchAll(/(?:src|href)="(\/[^"{}]+)"/g)) assert.equal((await request(match[1])).status, 200, match[1]);
...
test('service worker installs and serves the shell with network unavailable', async () => {
  ...
  const context = {
    URL, Request,
    self: { location: { origin: 'https://hyprland.test' }, clients: { claim: async () => {} }, addEventListener: (name, fn) => { handlers[name] = fn; } },
    caches: { open: async () => ({ put: async (key, value) => entries.set(key, value), match: async key => entries.get(key)?.clone() }), keys: async () => [], delete: async () => {} },
    fetch: async req => { if (offline) throw Error('Offline'); return worker.fetch(req); },
  };
  // Browser Request resolves relative URLs against the worker's origin.
  context.Request = class extends Request { constructor(url, init) { super(new URL(url, 'https://hyprland.test'), init); } };
  vm.runInNewContext(await (await request('/sw.js')).text(), context);
  let completion;
  handlers.install({ waitUntil: promise => { completion = promise; } });
  await completion;
  offline = true;
  handlers.fetch({ request: new Request('https://hyprland.test/'), respondWith: promise => { completion = promise; } });
