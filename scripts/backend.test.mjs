...
test('home widgets expose live host metrics and sanitized CodexBar windows',async()=>{
 const d=await api('widgets');assert.ok(d.metrics.memory_total>0);assert.ok(d.metrics.cores>0);assert.ok(d.metrics.updated_at>0);
 assert.ok(d.codexbar.providers.length===2);
 for(const p of d.codexbar.providers){assert.ok(['codex','claude'].includes(p.id));assert.equal(p.accountEmail,undefined);assert.equal(p.identity,undefined);for(const w of p.windows)assert.ok(w.used_percent>=0&&w.used_percent<=100)}
});
...
test('image uploads preserve bytes privately and reject invalid or oversized files',async()=>{
 ...
 const upload=body=>fetch(base+'/api/uploads/images',{method:'POST',headers:{'X-Hyprland-Client':'1','Content-Type':'image/png'},body});
 const bytes=Buffer.concat([Buffer.from('89504e470d0a1a0a','hex'),Buffer.alloc(65536,42)]);
 const response=await upload(bytes);assert.equal(response.status,200,await response.clone().text());
 const {path}=await response.json();assert.match(path,new RegExp('^'+process.env.HOME+'/.local/share/omarchy-remote/uploads/image-[a-f0-9-]+\\.png$'));
 try{assert.deepEqual(await readFile(path),bytes);assert.equal((await stat(path)).mode&0o777,0o600)}finally{await unlink(path)}
 ...
 assert.equal((await fetch(base+'/api/uploads/images',{method:'POST',body:bytes})).status,403);
});
...
test('Files browses, uploads, previews and creates folders without overwriting',async()=>{
 ...
 const request=(route,params,options={})=>fetch(base+'/api/'+route+'?'+new URLSearchParams(params),{...options,headers:{'X-Hyprland-Client':'1'}});
 try{
  await writeFile(folder+'/.hidden','hidden');await symlink('/etc',folder+'/outside');
  const bytes=Buffer.from('A real phone upload\n<script>plain text</script>\n');
  let r=await request('files/upload',{path:folder,name:'hello.txt'},{method:'POST',body:bytes});assert.equal(r.status,200);
  assert.deepEqual(await readFile(folder+'/hello.txt'),bytes);
  r=await request('files/upload',{path:folder,name:'hello.txt'},{method:'POST',body:'replacement'});assert.equal(r.status,400);assert.deepEqual(await readFile(folder+'/hello.txt'),bytes);
  r=await request('files/upload',{path:folder,name:'../escape'},{method:'POST',body:'bad'});assert.equal(r.status,400);
  r=await request('files/content',{path:folder+'/outside/passwd'});assert.equal(r.status,400);
  r=await request('files',{path:folder});let listing=await r.json();assert.deepEqual(listing.entries.map(e=>e.name),['hello.txt']);
  r=await request('files',{path:folder,hidden:true});listing=await r.json();assert.equal(listing.entries.length,2);
  await api('files/folders',{path:folder,name:'Phone photos'});
  r=await request('files',{path:folder});listing=await r.json();assert.equal(listing.entries[0].name,'Phone photos');assert.equal(listing.entries[0].directory,true);
  r=await request('files/content',{path:folder+'/hello.txt'});assert.equal(r.status,200);assert.equal(r.headers.get('content-type'),'application/octet-stream');assert.equal(await r.text(),bytes.toString());
  r=await request('files/upload',{path:folder,name:'too-big'},{method:'POST',body:Buffer.alloc(25*1024*1024+1)});assert.equal(r.status,413);
...
for(const [app,marker] of [['btop','CPU'],['services','Units'],['lazydocker','Containers'],['dua','mark-move']])test(`${app} launches separately with reconnect and isolated close`,async()=>{
 ...
 try{
  ...
  await api(`terminal/${monitor.id}/close`,{});await c.wait(m=>m.type==='exit');assert.equal((await api('terminal/session',{id:shell.id})).resumed,true);
 }finally{c.ws.close();await api(`terminal/${monitor.id}/close`,{});await api(`terminal/${shell.id}/close`,{})}
});
...
test('Files Trash is recoverable and contains the selected fixture bytes',async()=>{
 ...
 const name=basename(folder)+'.txt';const trash=(process.env.XDG_DATA_HOME||process.env.HOME+'/.local/share')+'/Trash';
 try{
  await writeFile(folder+'/'+name,'recoverable fixture');
  const result=await api('files/operate',{action:'trash',paths:[folder+'/'+name]});assert.equal(result.completed.length,1);assert.deepEqual(result.errors,[]);
  const infos=(await readdir(trash+'/info')).filter(n=>n.startsWith(name)&&n.endsWith('.trashinfo'));
  assert.equal(infos.length,1);const info=infos[0];assert.match(await readFile(trash+'/info/'+info,'utf8'),/\[Trash Info\]/);
  assert.equal(await readFile(trash+'/files/'+info.slice(0,-10),'utf8'),'recoverable fixture');
 }finally{
  // Remove only this uniquely named QA item, never empty the user's Trash.
  for(const info of (await readdir(trash+'/info').catch(()=>[])).filter(n=>n.startsWith(name)&&n.endsWith('.trashinfo'))){await rm(trash+'/files/'+info.slice(0,-10),{force:true});await rm(trash+'/info/'+info,{force:true})}
