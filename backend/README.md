...
- `WS /api/herdr/ws`: snapshots (checked every 1.2 seconds), selected pane output (read every 16 ms for a second after each keystroke and every ~100 ms otherwise, sent only when its text changes; each update names the pane's `foreground` programs and whether a `fullscreen` program such as Vim owns the screen), heartbeat/error messages, and acknowledged input. Output is the pane's last 300 lines; send `{ "type": "history", "deep": true }` while the reader is scrolled back to receive up to 1,000 (Herdr's read limit, re-read every ~0.5 seconds when idle), and `deep: false` to return to 300. Send `select` with `pane_id`, then `input` with `id`, `pane_id`, `text`, and `keys`. Input is rejected if its pane no longer matches the selected pane. Pending input is never replayed after reconnection; an interrupted acknowledgement is reported as uncertain.
