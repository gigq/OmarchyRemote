...
**CodexBar.** Tap its Home widget or open CodexBar from the launcher to inspect Codex and Claude usage, banked reset counts and expiry, credits, service status, and local token-cost history. Details depend on what the host CLI reports; reset credits are read-only. App providers can register their own linked Home widgets. Herd uses the same system: its widget opens individual project threads, prioritizes attention items, and shows working threads when none need attention.
