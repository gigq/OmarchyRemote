...
`android/` builds an Android 11+ native host for the same shell and backend. The initial implementation includes trusted shell messaging, saved hosts and preferences, native keyboard/battery integration, device weather location, Android file upload/save pickers, and independent browser WebViews. Full iPhone feature parity is **not yet verified**; [the Android parity checklist](docs/android-parity.md) tracks the remaining work.
